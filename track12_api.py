...
def log_entry(employee_id, roi_id):
    ...
    data = {
        employee_id: 1,  # Dynamic key for employee_id and its status (1 for entry)
        "employee_id": employee_id,
        "status": 1,  # Status: 1 indicates entry
        "time_stamp": timestamp,
        "roi_id": roi_id
    }
    
    try:
        # Insert a new document for each entry event
...
def log_exit(employee_id, last_seen_time, roi_id):
    ...
    # Data to be inserted into MongoDB
    data = {
        employee_id: 0,  # Dynamic key for employee_id and its status (0 for exit)
        "employee_id": employee_id,
        "status": 0,  # Status: 0 indicates exit
        "time_stamp": timestamp,
        "roi_id": roi_id
    }
    
    try:
        # Insert a new document for each exit event
        attendance_collection.insert_one(data)
...
def load_config_from_api(api_url):
    """
    Fetch configuration from a provided API URL.
    """
    try:
        ...
        print(f"Failed to fetch configuration from API: {e}")
        return None

def main():
    api_url = "http://192.168.1.46:5003"  # Replace with your actual API URL
    ...
    # Load known faces and names from the provided employee data
    ...
    for camera_id, camera_info in camera_data.items():
        print(f"Processing video from Camera {camera_id}")
